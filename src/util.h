...
uint16_t BufferPacket(uint8_t* buffer, Sensor** sensors, uint32_t sensorsSize) {  
    ...
    uint32_t head = 2; //Reserve 2 bytes for packet length - Needs to be able to be longer than 256 B/C of ECVT packets :/

    for(uint16_t i = 0; i < sensorsSize; i++) {
      head += sensors[i]->readPacketBlock(&buffer[head]); 
      //Todo: Make sure packet size is less than max so we're not overflowing.
    }

    //Set length, then calc and set checksum.
    head += 2;
    buffer[0] = (head >> 8);
    buffer[1] = head;

    buffer[head - 2] = 0xFF;
    buffer[head - 1] = CalcChecksum(buffer, head - 2);;
